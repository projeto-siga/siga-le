...
public class MesaGet implements IMesaGet {
	...
	public enum MarcadorEnum {
		...
		//
		A_ELIMINAR(7, "A Eliminar", "inbox", "", GrupoDeMarcadorEnum.OUTROS),
		//
	...
	public void run(MesaGetRequest req, MesaGetResponse resp) throws Exception {
		...
		try (ExDB db = ExDB.create(false)) {
			...
			List<Object[]> l = db.listarDocumentosPorPessoaOuLotacao(
					cadastrante, cadastrante.getLotacao());
			...
			resp.list = listarReferencias(TipoDePainelEnum.UNIDADE, map,
					cadastrante, cadastrante.getLotacao(), u,
					db.consultarDataEHoraDoServidor());
		}
	...
	public static List<MesaItem> listarReferencias(TipoDePainelEnum tipo,
			...
			DpLotacao unidade, Usuario usuario, Date currentDate) {
		...
		for (ExMobil mobil : references.keySet()) {
			...
			for (MeM tag : references.get(mobil)) {
				...
				t.titulo = Utils.calcularTempoRelativo(tag.marca
						.getDtIniMarca());
