...
public class ExMobilVO extends ExVO {
	static Logger log = Logger.getLogger(ExMobilVO.class.getCanonicalName());
	...
	public ExMobilVO(ExMobil mob, DpPessoa cadastrante, DpPessoa titular,
			DpLotacao lotaTitular, boolean completo) {
		this(mob, cadastrante, titular, lotaTitular, completo, null, false);
	}

	public ExMobilVO(ExMobil mob, DpPessoa cadastrante, DpPessoa titular,
			DpLotacao lotaTitular, boolean completo, Long tpMov,
			boolean movAssinada) {
		this.sigla = mob.getSigla();
		this.isGeral = mob.isGeral();
		this.id = mob.getId();

		this.podeTramitar = Ex.getInstance().getComp()
		...
		for (ExMobil m : mob.getApensosExcetoVolumeApensadoAoProximo()) {
			if (m.isEliminado())
				continue;
			apensos.add(new ExMobilVO(m, cadastrante, titular, lotaTitular,
					false));
		}
		log.debug(mob.getExDocumento().getCodigoString() + ": aExibir - mobil "
				+ mob.getNumSequencia() + " - adicao apensos: "
				+ (System.currentTimeMillis() - tempoIni));

		tempoIni = System.currentTimeMillis();
		for (ExDocumento d : mob.getExDocumentoFilhoSet()) {
			if (d.isExpediente())
				expedientesFilhosNaoCancelados.add(new ExDocumentoVO(d, null,
		...
		log.debug(mob.getExDocumento().getCodigoString() + ": aExibir - mobil "
				+ mob.getNumSequencia() + " - adicao filhos: "
				+ (System.currentTimeMillis() - tempoIni));

		tempoIni = System.currentTimeMillis();
		addAcoes(mob, titular, lotaTitular);
		log.debug(mob.getExDocumento().getCodigoString() + ": aExibir - mobil "
				+ mob.getNumSequencia() + " - adicao acoes: "
				+ (System.currentTimeMillis() - tempoIni));
			...
			for (ExMovimentacao mov : mob.getMovimentacoesPorTipo(tpMov)) {
				if (!movAssinada) {
					if (!mov.isAssinada() && !mov.isCancelada())
						movs.add(new ExMovimentacaoVO(this, mov, cadastrante,
								titular, lotaTitular));
				} else {
					if (mov.isAssinada())
						movs.add(new ExMovimentacaoVO(this, mov, cadastrante,
		...
		if (mob.doc().isEletronico()) {
			for (ExMovimentacao mov : mob.getAnexosNaoAssinados())
				anexosNaoAssinados.add(new ExMovimentacaoVO(this, mov,
						cadastrante, titular, lotaTitular));

			for (ExMobil juntado : mob.getJuntados())
				if (juntado.doc().isPendenteDeAssinatura())
					expedientesJuntadosNaoAssinados.add(new ExMobilVO(juntado,
							cadastrante, titular, lotaTitular, false));
		...
		if (mob.getPendenciasDeColaboracao() != null) {
			for (ExMovimentacao mov : mob.getPendenciasDeColaboracao()) {
				ExMovimentacaoVO m = new ExMovimentacaoVO(this, mov,
						cadastrante, mov.getSubscritor(),
		...
		for (ExMovimentacaoVO movVO : movs) {
			...
			if (i == movs.size()
				...
				if (i == movs.size() || movVOUlt == null)
					movVOUlt = movVO;
				SigaCalendar lIni = new SigaCalendar(
						movVOIni.dtIniMov.getTime());
				SigaCalendar lFim = new SigaCalendar(
						movVOUlt.dtIniMov.getTime());
		...
		log.debug(mob.getExDocumento().getCodigoString() + ": aExibir - mobil "
				+ mob.getNumSequencia() + " - adicao movs: "
				+ (System.currentTimeMillis() - tempoIni));
	...
	private void addAcoes(ExMobil mob, DpPessoa titular, DpLotacao lotaTitular) {
		if (!mob.isGeral()) {
			...
			addAcao("page_white_add", "Incluir _Documento",
					"/app/expediente/doc", "editar", Ex.getInstance().getComp()
							.podeIncluirDocumento(titular, lotaTitular, mob),
					null, "criandoAnexo=true&mobilPaiSel.sigla=" + getSigla(),
					null, null, null);

			if (mob.temAnexos()) {
				addAcao("script_key", "Assinar Anexos "
						+ (mob.isVia() ? "da Via" : "do Volume"),
	...
	public String getMarcadoresEmHtml(DpPessoa pess, DpLotacao lota) {
		...
		if (pess != null && lota != null) {
			...
			// if (mar.getCpMarcador().getIdMarcador() !=
			// CpMarcador.MARCADOR_EM_TRANSITO
			// && mar.getCpMarcador().getIdMarcador() !=
			...
			if (sb.length() == 0) {
				...
				// if (sb.length() > 0)
				// sb.append(", ");
				// if ((mar.getDpLotacaoIni() != null && lota
				// .getIdInicial().equals(
